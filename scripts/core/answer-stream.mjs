...
export function createAnswerStream() {
  let state = { text: "", phase: "starting", revision: 0 };
  let terminal = null;
  const clients = new Set();
  const frame = (type, data) => `id: ${state.revision}\nevent: ${type}\ndata: ${JSON.stringify(data)}\n\n`;
  const write = (res, type, data) => {
    if (res.destroyed || res.writableLength > MAX_TEXT * 2) return res.destroy();
  ...
  return {
    ...
    publish(event) {
      ...
      if (event.type === "status" && phases.has(event.phase)) {
        ...
        const text = event.text.slice(0, MAX_TEXT);
    ...
    subscribe(res) {
      ...
      const heartbeat = setInterval(() => {
        if (res.writableLength > MAX_TEXT * 2) res.destroy();

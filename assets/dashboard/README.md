...
Public requests receive only the GitHub login page until OAuth succeeds. OAuth `state` is signed, and the authorized GitHub login is recorded in a signed, `HttpOnly`, `Secure`, `SameSite=Lax` session cookie. Only `MY_WIKI_ADMIN_GITHUB_LOGIN` is allowed through; every authorized request uses the same `MY_WIKI_VAULT` as the local Dashboard. Requests through `127.0.0.1` continue to open the same Dashboard without the login gate.
